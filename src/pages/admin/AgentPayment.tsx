...
import {
  ArrowLeft, Globe, DollarSign, Percent, CreditCard,
...
export default function AgentPayment() {
  ...
  const { loading, result, processPayment, clearResult } = useAgentStore();
  ...
  const handleSubmit = async () => {
    if (!userId || !amount || !paymentMethod) return;
    await processPayment(agentId, {
      userId,
  ...
  useEffect(() => {
    if (result && result.success) {
      const t = setTimeout(() => { clearResult(); setUserId(""); setAmount(""); setCommissionPercent("0"); }, 4000);
      return () => clearTimeout(t);
    }
  }, [result, clearResult]);

  const canSubmit = userId && amount && paymentMethod && Number(amount) > 0 && !loading;
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-text-primary">Global Payment</h1>
          <p className="text-text-secondary text-sm mt-0.5">Process a global payment and delivery for the user</p>
        </div>
      </div>
      ...
      <Card className="p-6 space-y-5">
        <div className="flex items-center gap-2 pb-4 border-b border-border">
          ...
          <h2 className="text-lg font-bold text-text-primary">Payment Processing</h2>
        </div>
        ...
        <div>
          ...
          <input
            ...
            min="0"
          />
          <p className="text-[10px] text-text-subtle mt-1">Amount to debit from user for the payment</p>
        </div>
